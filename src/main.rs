...
use zon::{
    ...
    http,
    middleware::{HttpServiceExt as _, MapResponseService},
    router::{get, Router},
...
async fn async_main() -> Result<(), BoxError> {
    ...
    let svc = Arc::new(
        Router::new()
            ...
            .route(
                "/static/{*rest}",
                MapResponseService::new(
                    ServeDir::new("static").nested(1).fallback(routes::page_not_found.into_svc()),
                    |res: http::Response<_>| async move { res.map(Body::new) },
                ),
            )
